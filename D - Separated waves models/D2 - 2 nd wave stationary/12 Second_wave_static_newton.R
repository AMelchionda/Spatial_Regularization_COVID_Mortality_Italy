...
observations <- rowSums(mainland[,12:26])
...
observations <- rowSums(island[,12:26])
...
observations <- rowSums(mainland[,68:78])
...
observations <- rowSums(island[,68:78])
...
observations <- rowSums(mainland[,68:78]) - rowSums(mainland[,12:26])
...
observations <- rowSums(island[,68:78]) - rowSums(island[,12:26])
...
md_1st_solution[["optimization"]][["lambda_solution"]]# image(id_solution$fit.FEM)

#### Save results in a .RData file -------------------------------------------

setwd('./Results/Second wave')
save(mp1_solution, ip1_solution, 
     mc1_solution, ic1_solution, 
     md1_solution, id1_solution, 
     file = 'solutions_wave2_newton.RData')
